...
import { Delete as DeleteIcon, Edit as EditIcon, PushPin as PinIcon, Add as AddIcon } from "@mui/icons-material";
...
const stringifyItems = (items: Notice[] | Event[]): string => {
  return items.map((item) => `${item.id}:::${item.text}:::${item.isPinned}:::${item.date}`).join("|||");
};
...
const TourInfoManagement: React.FC = () => {
  ...
  const handleSaveEdit = () => {
    ...
    if (activeTab === 0) {
      // 更新通知
      const updatedNotices = notices.map((notice) => (notice.id === editItem.id ? (updatedItem as Notice) : notice));
      setNotices(updatedNotices);
      updateTourInfo({ latestNotice: stringifyItems(updatedNotices) });
    } else {
      // 更新事件
      const updatedEvents = events.map((event) => (event.id === editItem.id ? (updatedItem as Event) : event));
      setEvents(updatedEvents);
      updateTourInfo({ latestNewsEvent: stringifyItems(updatedEvents) });
    }
  ...
  const handleTogglePin = (id: string) => {
    if (activeTab === 0) {
      ...
      // 更新事件的置顶状态
      const updatedEvents = events.map((event) => (event.id === id ? { ...event, isPinned: !event.isPinned } : event));
      setEvents(updatedEvents);
      updateTourInfo({ latestNewsEvent: stringifyItems(updatedEvents) });
    }
  ...
  return (
    <Box p={3}>
        ...
        <Box p={3}>
          ...
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid item xs={12} md={2}>
              <FormControlLabel
                control={<Switch checked={isPinned} onChange={(e) => setIsPinned(e.target.checked)} color="primary" />}
                label="Pin to Top"
              />
            </Grid>
            <Grid item xs={12} md={2}>
      ...
      <Dialog open={dialogOpen} onClose={handleCancelEdit} maxWidth="md" fullWidth>
        <DialogTitle>Edit {activeTab === 0 ? "Notice" : "Event"}</DialogTitle>
        <DialogContent>
          <Box py={1}>
            <TextField
              fullWidth
        ...
        <DialogActions>
          ...
          <Button onClick={handleSaveEdit} color="primary" variant="contained" disabled={!newItemText.trim()}>
            Save
          </Button>
        </DialogActions>
      </Dialog>
